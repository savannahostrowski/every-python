...
@patch("every_python.main.build_python")
@patch("every_python.main._resolve_ref")
def test_install_without_ccache(
    mock_resolve: Mock, mock_build: Mock, tmp_path: Path
) -> None:
    mock_resolve.return_value = "abc123def456"
    mock_build.return_value = tmp_path / "abc123def456"

    result = runner.invoke(app, ["install", "main", "--no-ccache"])

    assert result.exit_code == 0
    mock_build.assert_called_once_with("abc123def456", BuildOptions(ccache=False))
...
@patch("every_python.main.platform.system", return_value="Linux")
def test_missing_ccache_is_ignored_in_auto_mode(
    _mock_platform: Mock, _mock_which: Mock
) -> None:
    assert _get_ccache_env() is None


@patch("every_python.main.shutil.which", return_value=None)
@patch("every_python.main.platform.system", return_value="Linux")
def test_missing_required_ccache_exits(_mock_platform: Mock, _mock_which: Mock) -> None:
    with pytest.raises(typer.Exit):
        _get_ccache_env(required=True)


@patch("every_python.main.platform.system", return_value="Windows")
def test_windows_is_ignored_in_auto_mode(_mock_platform: Mock) -> None:
    assert _get_ccache_env() is None


@patch("every_python.main.platform.system", return_value="Windows")
def test_required_ccache_on_windows_exits(_mock_platform: Mock) -> None:
    with pytest.raises(typer.Exit):
        _get_ccache_env(required=True)
